...
if (!RowtDeepLink) {
  console.warn('[Rowt SDK] Native module not available. Deep link handling will not work.');
}
...
export class NativeRowtConsole {
  ...
  static async getInitialDeepLink(): Promise<string | null> {
    if (!RowtDeepLink) {
      console.warn('[Rowt SDK] Cannot get initial deep link - native module not available');
      return null;
    }
  ...
  static addDeepLinkListener(handler: (url: string) => void): DeepLinkSubscription {
    if (!eventEmitter) {
      console.warn('[Rowt SDK] Cannot add listener - native module not available');
      return { remove: () => {} };
    }
    
    const subscription = eventEmitter.addListener('onRowtDeepLinkReceived', (event) => {
      handler(event.url);
    });
